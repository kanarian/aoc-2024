...
for val in vals:
    ...
    if z[0] >= 100 or z[1] >= 100:
        continue
        # print("Cannot solve without pressing a button more than 100 times")
    elif np.abs(z[0]-round(z[0])) > 1e-10 or np.abs(z[1] - round(z[1])) > 1e-10:
        print("Not a whole solution,",z)
        continue
    else:
        print(f"z={z}")
        print(f"Can Solve by pressing A {round(z[0])} times and B {round(z[1])} times\n")
        # round to nearest integers
        res = round(z[0])*3+round(z[1])*1
        tokens += res
