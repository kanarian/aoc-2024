...
tokens = 0
for val in vals:
    z = re.match(r"Button A: X\+(\d+), Y\+(\d+)\nButton B: X\+(\d+), Y\+(\d+)\nPrize: X=(\d+), Y=(\d+)",val).groups()
    (AX, AY, BX, BY, PX, PY) = map(int,z)
    PX=10000000000000+PX
    PY=10000000000000+PY

    A = np.matrix_transpose([[AX,AY],[BX,BY]])
    assert np.linalg.matrix_rank(A) == 2, "Non-rank 2 matrix!"
    z = np.linalg.solve(A,np.array([PX,PY]))

    if np.abs(z[0]-np.round(z[0])) > 1e-2 or np.abs(z[1] - np.round(z[1])) > 1e-2:
        # print("Not a whole solution,",z)
        continue
    else:
        print(f"z={z}")
        print(f"Can Solve by pressing A {np.round(z[0])} times and B {np.round(z[1])} times\n")
        a_press = np.round(z[0])
        b_press = np.round(z[1])
        assert a_press*AX+b_press*BX == PX, f"Something went wrong, {a_press*AX+b_press*BX} != {PX}"
        assert a_press*AY+b_press*BY == PY, f"Something went wrong, {a_press*AY+b_press*BY} != {PY}"
    
        tokens += a_press*3 + b_press*1
